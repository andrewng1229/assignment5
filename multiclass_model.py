...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model.add(keras.layers.Dense(output_length, input_shape=(input_length,), activation=activation_f))
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    #add a hidden layer with parameters
    model.add(keras.layers.Dense(hidden_layer_size, input_shape=(input_length,), activation=activation_func_array[0]))
    
    #add the output layer with parameters
    model.add(keras.layers.Dense(output_length, activation=activation_func_array[1]))
